...
function Find-Mpiexec {
  if ($env:AEGIS_MPIEXEC -and (Test-Path $env:AEGIS_MPIEXEC)) {
    return (Resolve-Path $env:AEGIS_MPIEXEC).Path
  }
  Refresh-Path
  foreach ($name in @("mpiexec", "mpiexec.exe", "mpirun", "mpirun.exe")) {
    $cmd = Get-Command $name -ErrorAction SilentlyContinue
    if ($cmd) { return $cmd.Source }
  }
  $candidates = @(
    (Join-Path $env:MSMPI_BIN "mpiexec.exe"),
    (Join-Path ${env:ProgramFiles} "Microsoft MPI\Bin\mpiexec.exe"),
    (Join-Path ${env:ProgramFiles(x86)} "Microsoft MPI\Bin\mpiexec.exe")
  )
...
function Save-EnvFile {
  ...
  $mpiexec = if ($env:AEGIS_MPIEXEC) { $env:AEGIS_MPIEXEC } else { "" }
