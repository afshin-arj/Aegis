...
function Ensure-Ovito {
  ...
  $probe = & $venvPy -c "import ovito; print(getattr(ovito,'version', 'ok'))" 2>$null
  if ($LASTEXITCODE -eq 0) {
    Write-Ok ("OVITO Python module present ({0})" -f ($probe | Select-Object -Last 1))
    return
  }
...
function Save-EnvFile {
  $lines = @(
    ...
    ("`$env:AEGIS_OVITO_BIN = '{0}'" -f $env:AEGIS_OVITO_BIN),
